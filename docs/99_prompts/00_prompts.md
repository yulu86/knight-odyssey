...
3. 设计实现方案
3. 设计每个task的具体实现步骤
6. 等待我确认实现方案准确无误后，输出手把手详细TDD开发指导到 docs/04_hands_by_hands/ 
要求:
- 输出到一份文档中
- 不要完全隔离测试和实现的步骤，而是严格使用TDD微循环
- 优先输出详细指导，让我完成.tscn开发/project.godot配置，再进行gdscript开发
约束：
...
```
请与我与我协同开发:
1. 从 docs/04_hands_by_hands/03_关卡系统基础框架开发指导.md 下获取待开发的指导，并与我确认
2. 等待我确认后，切换到开发分支
